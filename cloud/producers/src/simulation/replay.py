...
- When MINIO_ENDPOINT is unset or empty, connects to real AWS S3 using the
  ECS task IAM role (no explicit credentials needed).
- When MINIO_ENDPOINT is set, behaves identically to the local version.
"""
import io
import logging
import os
...
def _list_keys_by_date() -> dict[str, list[str]]:
    ...
    if not keys:
        logger.warning("No Parquet files in bucket — run scripts/historical_loader.py first")
        return {}
...
async def replay_stream():
    """Async generator yielding SensorReadings replayed from S3/MinIO Parquet
